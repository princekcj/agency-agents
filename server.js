...
// Strip any character outside printable ASCII (0x20-0x7E) to prevent ByteString errors in HTTP headers.
    ...
    for (const file of files) {
      try {
        ...
        agents.push({
          ...
          emoji: parsed.data.emoji || '\u{1F916}',
...
app.get('/api/agents/:division/:slug', (req, res) => {
  ...
  const agent = getAgents().find(a => a.division === division && a.slug === slug);
...
app.get('/api/stats', (req, res) => {
  const agents = getAgents();
  res.json({ total: agents.length, divisions: Object.keys(DIVISIONS).length });
});
...
app.post('/api/tts', async (req, res) => {
  ...
  try {
    const response = await fetch(
      ...
      {
        method: 'POST',
        headers: { 'xi-api-key': sanitizeHeaderValue(apiKey), 'Content-Type': 'application/json' },
        body: JSON.stringify({
          text,
          model_id: 'eleven_multilingual_v2',
          voice_settings: { stability: 0.52, similarity_boost: 0.88, style: 0.60, use_speaker_boost: true },
        }),
      }
    );
    if (!response.ok) {
...
app.get('/api/activities', (req, res) => {
  res.json(activityLog.slice(0, 249));
});
...
app.get('/api/stats/24h', (req, res) => {
  ...
  const byDivision = {};
  for (const e of recent) byDivision[e.division] = (byDivision[e.division] || 0) + 1;
  const agentCounts = {};
  ...
  const byHour = [];
  for (let h = 0; h < 24; h++) {
    const start = new Date(cutoff + h * 3600000);
    const end = new Date(cutoff + (h + 1) * 3600000);
    byHour.push({ hour: start.getHours(), count: recent.filter(e => { const t = new Date(e.startedAt).getTime(); return t >= start && t < end; }).length });
  }
...
function loadSchedules() {
  try { if (fs.existsSync(SCHEDULES_FILE)) schedules = JSON.parse(fs.readFileSync(SCHEDULES_FILE, 'utf8')); }
  catch { schedules = []; }
}
...
app.post('/api/scheduler/key', requireSchedulerToken, (req, res) => {
  ...
  res.json({ encrypted: encrypt(key) });
});
...
app.post('/api/scheduler/schedules', requireSchedulerToken, (req, res) => {
  ...
  if (!name || !cronExpr || !pipeline || !encryptedKey) return res.status(400).json({ error: 'name, cron, pipeline, encryptedKey required' });
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    if (!upstream.ok) {
      ...
      let errMsg = errText;
      try { const errJson = JSON.parse(errText); errMsg = errJson.error?.message || errJson.error || errJson.message || errText; } catch {}
...
app.post('/api/install', async (req, res) => {
  ...
  const toolArgs = [];
  ['npm', 'git', 'node', 'python'].forEach(t => { toolArgs.push('--tool'); toolArgs.push(t); });
  const install = spawn('bash', ['scripts/install.sh', '--agent', agentArg, '--no-interactive', ...toolArgs]);
  ...
  install.on('close', code => broadcast({ type: 'install_done', exitCode: code, agentId: agentArg }));
});
...
async function runSchedule(schedule) {
  ...
  for (let i = 0; i < pipeline.length; i++) {
    ...
    try {
      const userContent = i === 0 ? currentInput : `Previous agent output:\n\n${currentInput}\n\nContinue processing this according to your role.`;
      ...
      currentInput = data.choices?.[0]?.message?.content || '(no output)';
      results.push({ agentName: agent.name, agentEmoji: agent.emoji, output: currentInput });
    } catch (e) {
...
loadSchedules();
schedules.filter(s => s.active).forEach(registerCronJob);
console.log(`Loaded ${schedules.filter(s => s.active).length} schedule(s).`);

// -- OpenClaw convert (workspace generation only) ------------------------------
app.post('/api/openclaw/convert', (req, res) => {
  res.json({ ok: true });
  const convert = spawn('bash', ['scripts/convert.sh', '--tool', 'openclaw'], { cwd: __dirname, stdio: ['ignore', 'pipe', 'pipe'] });
...
// -- OpenClaw install (convert + install all agents) ---------------------------
app.post('/api/openclaw/install', (req, res) => {
  ...
  convert.on('close', convertCode => {
    ...
    const install = spawn('bash', ['scripts/install.sh', '--tool', 'openclaw', '--no-interactive'], { cwd: __dirname, stdio: ['ignore', 'pipe', 'pipe'] });
    install.stdout.on('data', d => broadcast({ type: 'install_log', text: d.toString() }));
    install.stderr.on('data', d => broadcast({ type: 'install_log', text: d.toString() }));
    install.on('close', code => {
      broadcast({ type: 'install_done', exitCode: code });

      // Create an individual activity entry for every deployed agent so the
      // dashboard shows one row per agent instead of one collective "All Agency Agents" entry.
      const now = new Date().toISOString();
      const status = code === 0 ? 'done' : 'error';
      const action = code === 0 ? 'Deployed to OpenClaw' : 'Deploy to OpenClaw failed';
      for (const agent of getAgents()) {
        activityLog.unshift({
          id: `install-${agent.id}-${Date.now()}-${Math.random().toString(36).slice(2, 5)}`,
          agentId: agent.id,
          agentName: agent.name,
          agentEmoji: agent.emoji || '\u{1F916}',
          division: agent.division,
          divisionColor: agent.divisionColor || '#dc2626',
          action,
          status,
          startedAt: now,
          completedAt: now,
      ...
      if (activityLog.length > 300) activityLog.length = 300;
      // Single broadcast of the full state rather than 249 separate agent_done events.
      broadcast({ type: 'activity_state', entries: activityLog.slice(0, 249) });
    });
  });
});
