    ...
    <div className="agent-card" onClick={() => onSelect(agent)}>
      <div className="agent-card-header">
        ...
        <div className="agent-card-info">
          ...
          <div className="agent-division" style={{ color: agent.divisionColor }}>{agent.divisionLabel}</div>
        </div>
      </div>
      {agent.description && (
      ...
      <div className="agent-detail" onClick={e => e.stopPropagation()}>
        <div className="detail-header">
          ...
          <div>
            ...
            <div className="detail-division" style={{ color: agent.divisionColor }}>{agent.divisionLabel}</div>
          </div>
          <button className="detail-close" onClick={onClose}><X size={14} /></button>
        </div>
        <div className="detail-body">
          {agent.description && <p className="detail-description">{agent.description}</p>}
          {agent.vibe && <p className="detail-vibe">"{agent.vibe}"</p>}
          <div className="detail-actions">
            <button className="detail-action-btn btn-brief" onClick={() => onSpeak(agent)}>
...
const ACTIVITY_QUIPS = [
  ...
  (name) => `${name}. Interesting. I've already assessed it. The assessment is favourable.`,
  (name) => `${name}. Now we're talking. Or rather, I am. I'm always talking.`,
];
  ...
  const { connected } = useWebSocket(useCallback((msg) => {
    ...
    if (msg.type === 'agent_done') {
      setActivities(prev => {
        ...
        return [msg.entry, ...prev].slice(0, 249);
      });
      refresh24h();
